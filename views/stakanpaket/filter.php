...
<div id="content"  role="main" class="content-register">
    <h2 class="h2">Выбор по параметрам</h2>

<?php if($products['notfound']): // если ничего не найдено ?>
    <?php
        $var1 = $products['notfound'];
    ...
    <div class="vid-sort">
        ...
        <div class="sort-wrap">
            <?php foreach($current_order['order_p'] as $key => $value): ?>
                ...
                <a href="?view=filter&amp;startprice=<?=$startprice?>&amp;endprice=<?=$endprice?>&amp;<?php foreach ($brand as $value_brand): ?>brand[]=<?=$value_brand?>&amp;<?php endforeach; ?>order=<?=$key?>&amp;page=<?=$page?>" class="sort-bot"><?=$value[0]?></a>
            <?php endforeach; ?>
        </div>
    </div>
