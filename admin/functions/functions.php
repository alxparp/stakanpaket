...
function del_product($product_id){
    $query = "DELETE FROM goods WHERE goods_id = $product_id";
    $res = mysql_query($query);
    ...
    if(mysql_affected_rows() > 0){
        ...
        $_SESSION['answer'] = "<div class='error'>Ошибка удаления товара!</div>";
        return false;
    }
}
/* ===Удаление категории=== */
...
function edit_product($id){
    ...
    $profile_auto = trim($_POST['profile_auto']);
    $diameter = trim($_POST['diameter']);
    $seasonality = trim($_POST['seasonality']);
    $index_auto = trim($_POST['index_auto']);
...
//            $_SESSION['edit_product']['res'] = "<div class='error'>У товара должно быть название</div>";
            $_SESSION['edit_product']['res'] = "<div class='error'>Заполните все поля со звездочкой</div>";
            return false;
    }else{
        ...
        $manufacturer = clear_admin($manufacturer);
        $model = clear_admin($model);
        $type_auto = clear_admin($type_auto);
        $width = clear_admin($width);
        $profile_auto = clear_admin($profile_auto);
        $diameter = clear_admin($diameter);
        $seasonality = clear_admin($seasonality);
        $index_auto = clear_admin($index_auto);
        $country = clear_admin($country);
        ...
        $query = "UPDATE specifications SET
        ...
        /* базовая картинка */
        $types = array("image/gif", "image/png", "image/jpeg", "image/jpg", "image/pjpeg", "image/x-png"); // массив допустимых расширений
        if($_FILES['baseimg']['name']){
            ...
            if(empty($error)){
                if(@move_uploaded_file($baseimgTmpName, "../userfiles/product_img/tmp/$baseimgName")){
                    resize("../userfiles/product_img/tmp/$baseimgName", "../userfiles/product_img/baseimg/$baseimgName", 180, 200, $baseimgExt);
...
//            $_SESSION['add_product']['res'] = "<div class='error'>У товара должно быть название</div>";
            $_SESSION['add_product']['res'] = "<div class='error'>Заполните все поля со звездочкой</div>";
            ...
            // Характеристики
            $_SESSION['add_product']['manufacturer'] = $manufacturer;
            $_SESSION['add_product']['model'] = $model;
            $_SESSION['add_product']['type_auto'] = $type_auto;
            $_SESSION['add_product']['width'] = $width;
            $_SESSION['add_product']['profile_auto'] = $profile_auto;
            $_SESSION['add_product']['diameter'] = $diameter;
            $_SESSION['add_product']['seasonality'] = $seasonality;
            $_SESSION['add_product']['index_auto'] = $index_auto;
            $_SESSION['add_product']['country'] = $country;
            return false;
    }else{
        $name = clear_admin($name);
        $keywords = clear_admin($keywords);
        ...
        if(mysql_affected_rows() > 0){
            ...
            if($_FILES['galleryimg']['name'][0]){
                ...
                $query = "INSERT INTO specifications (specification_id, manufacturer, model, type_auto, width, profile_auto, diameter, seasonality, index_auto, country)
                    VALUES ($id,'$manufacturer', '$model', '$type_auto', '$width', '$profile_auto', '$diameter', '$seasonality', '$index_auto', '$country')";
                $res = mysql_query($query) or die(mysql_error());
            }
            /* картинки галереи */
...
function insert_product($goods, $specification) {
    $query = "INSERT INTO goods (goods_id, name, price, rest, goods_brandid, hits, new, sale, date, visible)
                    VALUES ".implode(',',$goods);
    $res = mysql_query($query) or die(mysql_error());
    $id = mysql_insert_id();

    $query = "INSERT INTO specifications (specification_id, manufacturer, model, width, profile_auto, diameter, seasonality, index_auto, country)
                    VALUES ".implode(',',$specification);
    $res = mysql_query($query) or die(mysql_error());
}
// Добавление товара в базу
...
function get_specifications($goods_id){
    $query = "SELECT * FROM specifications WHERE specification_id = $goods_id";
    $res = mysql_query($query);
...
function update_contacts($first_contact, $second_contact, $third_contact, $email) {
    ...
    $res = mysql_query($query) or die(mysql_error());
    if(mysql_affected_rows() > 0){
        $_SESSION['edit_contacts']['res'] = "<div class='success'>Контакты обновлены!</div>";
        return true;
    }else{
        $_SESSION['edit_contacts']['res'] = "<div class='error'>Ошибка при редактировании контактов!</div>";
        return false;
    }
}
...
function get_contact(){
    $query = "SELECT * FROM contacts WHERE contacts_id = 1";
    $res = mysql_query($query);
